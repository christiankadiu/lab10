...
public final class LambdaUtilities {
   ...
   public static <T> List<Optional<T>> optFilter(final List<T> list, final Predicate<T> pre) {
       ...
       list.forEach(t -> lista.add(Optional.ofNullable(t).filter(pre)));
       return lista;
   }
   ...
   public static <R, T> Map<R, Set<T>> group(final List<T> list, final Function<T, R> op) {
       ...
       list.forEach(t -> {
           mappa.merge(op.apply(t), new HashSet<>(Collections.singleton(t)), (oldValues, newValues) -> {
   ...
   public static <K, V> Map<K, V> fill(final Map<K, Optional<V>> map, final Supplier<V> def) {
       ...
       for (final Map.Entry<K, Optional<V>> entry : map.entrySet()) {
           mappa.put(entry.getKey(), entry.getValue().orElse(def.get()));
       }
       return mappa;
   }
